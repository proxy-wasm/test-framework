...
pub fn generate_import_list(store: &Store, module: &Module, func_vec: Arc<Mutex<Vec<Extern>>>) 
...
fn get_hostfunc(store: &Store, import: &ImportType) -> Option<Func> {
    ...
    match import.name() {
        ...
        "proxy_log" => {Some(Func::wrap(
                &store, |caller: Caller<'_>, level: i32, message_data: i32, message_size: i32| -> i32 {
                    ...
                    let mem = match caller.get_export("memory") {
                        ...
                        _ => {println!("=>   Error: proxy_log cannot get_export \"memory\""); 
                    ...
                    unsafe {
                        ...
                        let string_msg = data.map(|string_msg| std::str::from_utf8(string_msg).unwrap());
                        let string_msg = match string_msg {
                            Some(s) => s,
                            _ => "invalid utf-8 slice"
                        };

                        if let Some(expect_log_string) = EXPECT.lock().unwrap().staged.get_expect_log(level) {
                            assert_eq!(string_msg.to_string(), expect_log_string);
                        }

                        println!("=>   proxy_log | Level: {} | Message: {}", level, string_msg);
                    }
        ...
        "proxy_get_current_time_nanoseconds" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, return_time: i32| -> i32 {
                    // Default Function: respond to proxy-wasm module with the current time 
                    // Expectation: respond with a pre-set expected time
                    let mem = match caller.get_export("memory") {
                        Some(Extern::Memory(mem)) => mem,
                        _ => {println!("=>   Error: proxy_get_current_time_nanoseconds cannot get export \"memory\"");
                              return Status::InternalFailure as i32;
                            }
                    };
                    
                    let time = match EXPECT.lock().unwrap().staged.get_expect_get_current_time_nanos() {
                    ...
                    unsafe {
                        let data = mem.data_unchecked_mut()
                            .get_unchecked_mut(return_time as u32 as usize..return_time as u32 as usize + 8);
        ...
        "proxy_set_tick_period_milliseconds" => {
            Some(Func::wrap(
                &store, |_caller: Caller<'_>, period: i32| -> i32 { 
                    ...
                    HOST.lock().unwrap().staged.set_tick_period_millis(period as u64);
                    if let Some(tick_period_millis) = EXPECT.lock().unwrap().staged.get_expect_set_tick_period_millis() {
        ...
        "proxy_get_configuration" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, return_buffer_data: i32, return_buffer_size: i32| -> i32 {
                    // Default Function: 
                    // Expectation: 
        ...
        "proxy_get_buffer_bytes" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, buffer_type: i32, start: i32, max_size: i32, 
                    ...
                    let mem = match caller.get_export("memory") {
                        Some(Extern::Memory(mem)) => mem,
                        _ => {println!("=>   Error: proxy_get_buffer_bytes cannot get export \"memory\"");
                                return Status::InternalFailure as i32;
                            }
                    };
                    ...
                    let malloc = match caller.get_export("malloc") {
                        ...
                        _ => {println!("=>   Error: proxy_get_buffer_bytes cannot get export \"malloc\"");
                              return Status::InternalFailure as i32;    
                        }
                    };

                    let response_body = match EXPECT.lock().unwrap().staged.get_expect_get_buffer_bytes(buffer_type) {
                        ...
                        None => {
                            ...
                            let host_buffer_bytes = HOST.lock().unwrap().staged.get_buffer_bytes(buffer_type);
                            if host_buffer_bytes.len() == (max_size - start) as usize {
                                buffer_bytes = host_buffer_bytes;
                            } else {
                                buffer_bytes = serial_utils::generate_random_string((max_size - start) as usize).as_bytes().to_vec();
                            }
                            buffer_bytes
                        } 
                    };
                    ...
                    unsafe {
                        let return_buffer_size_ptr = mem.data_unchecked_mut()
                            .get_unchecked_mut(return_buffer_size as u32 as usize..
                                return_buffer_size as u32 as usize + 4);

                        let return_buffer_data_ptr = mem.data_unchecked_mut()
                            .get_unchecked_mut(return_buffer_data as u32 as usize..
                                return_buffer_data as u32 as usize + 4);
    
                        // allocate memory and store buffer bytes
                        let buffer_data_add = malloc(response_body.len() as i32).unwrap() as u32 as usize;
                        let buffer_data_ptr = mem.data_unchecked_mut()
                            .get_unchecked_mut(buffer_data_add..buffer_data_add + response_body.len());
                        buffer_data_ptr.copy_from_slice(&response_body);
                         
                        return_buffer_size_ptr.copy_from_slice(&(response_body.len() as u32).to_le_bytes());
                        return_buffer_data_ptr.copy_from_slice(&(buffer_data_add as u32).to_le_bytes());
                    }
                    return Status::Ok as i32;
                }
            ))
        ...
        "proxy_get_header_map_pairs" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, map_type: i32, return_map_data: i32, return_map_size: i32| -> i32 {
                    ...
                    let mem = match caller.get_export("memory") {
                        Some(Extern::Memory(mem)) => mem,
                        _ => {println!("=>   Error: proxy_get_header_map_pairs cannot get export \"memory\"");
                              return Status::InternalFailure as i32;
                            }
                    };
                    ...
                    let malloc = match caller.get_export("malloc") {
                        Some(Extern::Func(func)) => func.get1::<i32, i32>().unwrap(),
                        _ => {println!("=>   Error: proxy_get_header_map_pairs cannot get export \"malloc\"");
                              return Status::InternalFailure as i32;    
                        }
                    };
                    
                    let serial_map = match EXPECT.lock().unwrap().staged.get_expect_get_header_map_pairs(map_type) {
                    ...
                    unsafe {        
                        let return_map_size_ptr = mem.data_unchecked_mut()
                            .get_unchecked_mut(return_map_size as u32 as usize..
                                return_map_size as u32 as usize + 4);
                        
                        let return_map_data_ptr = mem.data_unchecked_mut()
                            .get_unchecked_mut(return_map_data as u32 as usize..
                                return_map_data as u32 as usize + 4);
                    
                        let map_data_add = malloc(serial_map_size as i32).unwrap() as u32 as usize;
                        let map_data_ptr = mem.data_unchecked_mut()
                        ...
                        return_map_size_ptr.copy_from_slice(&(serial_map_size as u32).to_le_bytes());
                    }
                    return Status::Ok as i32;
                }
            ))
        ...
        "proxy_set_header_map_pairs" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, map_type: i32, map_data: i32, map_size: i32| -> i32 {
                    ...
                    let mem = match caller.get_export("memory") {
                        Some(Extern::Memory(mem)) => mem,
                        _ => {println!("=>   Error: proxy_set_header_map_pairs cannot get export \"memory\"");
                              return Status::InternalFailure as i32;
                            }
                    };
                    ...
                    unsafe {
                        ...
                        let header_map_ptr = mem.data_unchecked()
                            .get_unchecked(map_data as u32 as usize..
                                (map_data + map_size) as u32 as usize);

                        HOST.lock().unwrap().staged.set_header_map_pairs(map_type, 
                            serial_utils::deserialize_map(header_map_ptr).iter().map(|(k, v)| (k as &str, v as &str)).collect());

                        if let Some(expect_header_map) = EXPECT.lock().unwrap().staged.get_expect_set_header_map_pairs(map_type) {
        ...
        "proxy_get_header_map_value" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, map_type: i32, key_data: i32, key_size: i32, 
                    ...
                    let mem = match caller.get_export("memory") {
                        Some(Extern::Memory(mem)) => mem,
                        _ => {println!("=>   Error: proxy_get_header_map_value cannot get export \"memory\"");
                                return Status::InternalFailure as i32;
                            }
                    };
                    ...
                    let malloc = match caller.get_export("malloc") {
                        Some(Extern::Func(func)) => func.get1::<i32, i32>().unwrap(),
                        _ => {println!("=>   Error: proxy_get_header_map_value cannot get export \"malloc\"");
                                return Status::InternalFailure as i32;    
                        }
                    };    
                    ...
                    unsafe {
                        ...
                        let key_data_ptr = mem.data_unchecked()
                            .get(key_data as u32 as usize..)
                            .and_then(|arr| arr.get(..key_size as u32 as usize));
                        let string_key = key_data_ptr.map(|string_msg| std::str::from_utf8(string_msg).unwrap()).unwrap();

                        let string_value = match EXPECT.lock().unwrap().staged.get_expect_get_header_map_value(map_type, string_key) {
                            Some(expect_string_value) => expect_string_value,
                            None => {match HOST.lock().unwrap().staged.get_header_map_value(map_type, &string_key) {
                        ...
                        let value_data_add = malloc(string_value.len() as i32).unwrap() as u32 as usize;
                        let value_data_ptr = mem.data_unchecked_mut()
                            .get_unchecked_mut(value_data_add..value_data_add + string_value.len());
                        value_data_ptr.copy_from_slice((&string_value).as_bytes());
                        
                        let return_value_data_ptr = mem.data_unchecked_mut()
                            .get_unchecked_mut(return_value_data as u32 as usize..
                                return_value_data as u32 as usize + 4);

                        let return_value_size_ptr = mem.data_unchecked_mut()
                            .get_unchecked_mut(return_value_size as u32 as usize..
                                return_value_size as u32 as usize + 4);
                
                        return_value_data_ptr.copy_from_slice(&(value_data_add as u32).to_le_bytes());
                        return_value_size_ptr.copy_from_slice(&(string_value.len() as u32).to_le_bytes());
                    }
        ...
        "proxy_replace_header_map_value" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, map_type: i32, key_data: i32, key_size: i32, value_data: i32, value_size: i32| -> i32 {
                    ...
                    let mem = match caller.get_export("memory") {
                        Some(Extern::Memory(mem)) => mem,
                        _ => {println!("=>   Error: proxy_replace_header_map_value cannot get export \"memory\"");
                                return Status::InternalFailure as i32;
                            }
                    };
                    ...
                    unsafe {
                        ...
                        let value_data_ptr = mem.data_unchecked()
                            .get(value_data as u32 as usize..)
                            .and_then(|arr| arr.get(..value_size as u32 as usize));
                        let string_value = value_data_ptr.map(|string_msg| std::str::from_utf8(string_msg).unwrap()).unwrap();

                        EXPECT.lock().unwrap().staged.get_expect_replace_header_map_value(map_type, string_key, string_value);
                        HOST.lock().unwrap().staged.replace_header_map_value(map_type, string_key, string_value);
                    }
        ...
        "proxy_remove_header_map_value" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, map_type: i32, key_data: i32, key_size: i32| -> i32 {
                    ...
                    let mem = match caller.get_export("memory") {
                        Some(Extern::Memory(mem)) => mem,
                        _ => {println!("=>   Error: proxy_remove_header_map_value cannot get export \"memory\"");
                                return Status::InternalFailure as i32;
                            }
                    };
                    ...
                    unsafe {
                        ...
                        EXPECT.lock().unwrap().staged.get_expect_remove_header_map_value(map_type, string_key);
                        HOST.lock().unwrap().staged.remove_header_map_value(map_type, string_key);
                    }
        ...
        "proxy_add_header_map_value" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, map_type: i32, key_data: i32, key_size: i32, value_data: i32, value_size: i32| -> i32 {
                    ...
                    let mem = match caller.get_export("memory") {
                        Some(Extern::Memory(mem)) => mem,
                        _ => {println!("=>   Error: proxy_add_header_map_value cannot get export \"memory\"");
                                return Status::InternalFailure as i32;
                            }
                    };
                    ...
                    unsafe {
                        ...
                        EXPECT.lock().unwrap().staged.get_expect_add_header_map_value(map_type, string_key, string_value);
                        HOST.lock().unwrap().staged.add_header_map_value(map_type, string_key, string_value);
                    }
        ...
        "proxy_send_local_response" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, status_code: i32, _status_code_details_data: i32,
                         _status_code_details_size: i32, body_data: i32, body_size: i32, 
                    ...
                    let mem = match caller.get_export("memory") {
                        Some(Extern::Memory(mem)) => mem,
                        _ => {println!("=>   Error: proxy_send_local_response cannot get export \"memory\"");
                                return Status::InternalFailure as i32;
                            }
                    };
                    ...
                    unsafe {
                        ...
                        EXPECT.lock().unwrap().staged.get_expect_send_local_response(status_code, string_body, 
        ...
        "proxy_http_call" => {
            Some(Func::wrap(
                &store, |caller: Caller<'_>, upstream_data: i32, upstream_size: i32, headers_data: i32,
                         headers_size: i32, body_data: i32, body_size: i32, trailers_data: i32, 
                         trailers_size: i32, timeout: i32, return_token: i32| -> i32 {
                    ...
                    let mem = match caller.get_export("memory") {
                        Some(Extern::Memory(mem)) => mem,
                        _ => {println!("=>   Error: proxy_http_call cannot get export \"memory\"");
                                return Status::InternalFailure as i32;
                            }
                    };
                    ...
                    unsafe {
                        let upstream_data_ptr = mem.data_unchecked()
                        ...
                        let trailer_data_ptr = mem.data_unchecked()
                            .get_unchecked(trailers_data as u32 as usize..
                        ...
                        let token_id = match EXPECT.lock().unwrap().staged.get_expect_http_call(
                            string_upstream, header_data_ptr, string_body, trailer_data_ptr, timeout) {
                                Some(expect_token) => expect_token,
                                None => 0
                            };

                        let return_token_add = mem.data_unchecked_mut()
                            .get_unchecked_mut(return_token as u32 as usize..
                                return_token as u32 as usize + 4);
        ...
        "proxy_set_effective_context" => {
            Some(Func::wrap(
                &store, |_caller: Caller<'_>, context_id: i32| -> i32 {
                    // Default Function: 
                    // Expectation:
